...
def calculatePlantCapacityDerates(genFleet, weatherYears, metVars, compressedGens,cesmMembers=None):
    #Initialize derates to zero
    time_idx = metVars.time.sel(time=slice(weatherYears[0],weatherYears[-1]))
    
    if cesmMembers is None:
        try:
            if metVars['member_id'].values.size == 1:
                cesmMembers = [metVars['member_id'].values]
            else:
                cesmMembers = metVars.values
            #print(cesmMembers)
        except (RuntimeError, TypeError, NameError):
            print("No ensemble members")
            
    genFleet['GAMS Symbol'] = genFleet.index
    
    capDerates = pd.DataFrame(0,index=time_idx,columns=genFleet['GAMS Symbol'])
    ...
    if cesmMembers != None: 
        ...
        if len(cesmMembers) == 1:
            capDerates = derateExistingUnits(genFleet,capDerates,weatherYears,metVars,cesmMembers[0],compressedGens)
        else:
            ...
            for cesmMember in cesmMembers:
                ...
                capDerates = derateExistingUnits(genFleet,capDerates,weatherYears,metVars,cesmMember,compressedGens)
...
def techRowEligForDerates(genRow):
    return ((genRow['PlantType'] in ['CC','CTCCS','CCCCS'] and genRow['CoolingType'] in ['RC','DC']))
################################################################################

############### DERATE THERMAL POWER PLANTS ####################################
def derateExistingUnits(genFleet,capDerates,weatherYears,metVars,cesmMember,compressedGens):
    ...
    # If all old plants are retired, and only CCS is available
    if len(derates):
        derates = pd.concat(derates,axis=1)
    
    CCS_derate_elems = [calculateGenDerates_CCS(genFleet.iloc[i],derates,genFleet,metVars)\
                        for i in range(genFleet.shape[0]) if ('COMBINED' not in genFleet.iloc[i]['GAMS Symbol'] \
                                                              and\
                                                              rowEligForDerates(genFleet.iloc[i],newGenType=True))
                       ]
    if len(CCS_derate_elems):
        CCS_Derates = pd.concat(CCS_derate_elems,axis=1)
    
    # If all old plants are retired, and only CCS is available            
    if len(derates) and len(CCS_derate_elems):
        derates = pd.concat([derates,CCS_Derates],axis=1)
    elif len(derates):
        derates = derates
    elif len(CCS_derate_elems):
        derates = CCS_Derates
...
def rowEligForDerates(genRow,newGenType=False):
    if newGenType:
        return (genRow['PlantType'] in ['CCCCS'] and genRow['CoolingType'] in ['RC','DC'])
    else:    
        return ((genRow['PlantType'] in ['ST','CC'] and genRow['CoolingType'] in ['RC','DC']) or (genRow['PlantType'] == 'CT'))

def calculateGenDerates(metVars,genRow,rcDesign,dcDesign,coalRC,coalDC,ngRC,ngDC):
    #Get generator characteristics
    cool,pt,lat,lon = genRow['CoolingType'],genRow['PlantType'],genRow['lat'],genRow['lon']
    ...
    elif pt == 'CT': availCapac = derateCTs(unitMet)
    
    if 'GAMS Symbol' in genRow.index: availCapac.name = genRow['GAMS Symbol']
    else: availCapac.name = genRow.name


    return 1 - availCapac 

def calculateGenDerates_CCS(genRow,capDerates,genFleet,metVars):

    #Get tech's plant type, region, and cooling type
    pt,region,cool = genRow['PlantType'],genRow['region'],genRow['CoolingType']
    ...
    if gens.shape[0]>0:
        genDerates = capDerates[gens['GAMS Symbol']]
        capDerates_CCS = genDerates.mean(axis=1)
        capDerates_CCS.name=genRow['GAMS Symbol']
    else: #if no other gens of that type in region, drop that gen as a new tech option
        capDerates_CCS = pd.Series(np.zeros(len(metVars.time.values)),index=metVars.time.values,name=genRow['GAMS Symbol'])
    
    return capDerates_CCS

def loadCoeffs():
    baseDir = os.path.join('derates_inputs')
