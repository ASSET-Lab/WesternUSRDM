...
def addHourSubsets(db,hoursForCE,hydroGenMonthlyCE):
    ...
    if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame:
        for idx in hydroGenMonthlyCE.index: 
            monthHourSymbols = hoursForCE.loc[(hoursForCE.index.month==hydroGenMonthlyCE.loc[idx,'dt'].month) & (hoursForCE.index.year==hydroGenMonthlyCE.loc[idx,'dt'].year)].index.astype(str)
            addSet(db,monthHourSymbols,idx)
        hydroGenMonthlyCE.drop('dt',axis=1,inplace=True) #drop dt index column
