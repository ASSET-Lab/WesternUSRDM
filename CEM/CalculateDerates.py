...
def calculatePlantCapacityDerates(genFleet, newTechsCE, demand, weatherYears, cesmMembers, compressedGens):
    ...
    if cesmMembers != None: 
        #Load weather
        metVars = importCESMMetVars(weatherYears,cesmMembers[0])

        #Derate existing units
        if len(cesmMembers) == 1: capDerates = derateExistingUnits(genFleet,capDerates,metVars,compressedGens)
        else: sys.exit('HAVE NOT TESTED THE MULTIPLE ENSEMBLE MEMBERS IN PLANT DERATES')
            # capDeratesAll = list()
        ...
        #Derate new techs by creating versions w/ different cooling types & using locations
        capDeratesTechs,newTechsCE = calculateNewTechDerates(capDeratesTechs,newTechsCE,metVars)
...
def calculateNewTechDerates(capDeratesTechs,newTechsCE,metVars):
    #Split derated techs by cooling type
    newTechsCE = splitTechsByCT(newTechsCE)

    #Reinitialize derates to zero with new techs
    capDeratesTechs = pd.DataFrame(0,index=capDeratesTechs.index,columns=newTechsCE['GAMS Symbol'])

    #Set recirculating and dry cooling design assumptions
    rcDesign,dcDesign = setCoolingDesigns()

    #Load derate coefficients for coal & NG RC & DC units (using Aviva Loew paper)
    coalRC,coalDC,ngRC,ngDC = loadCoeffs()

    #Calculate derates for eligible units
    deratesTechs = [calculateGenDerates(metVars,newTechsCE.iloc[i],rcDesign,dcDesign,coalRC,coalDC,ngRC,ngDC) 
                for i in range(newTechsCE.shape[0]) if rowEligForDerates(newTechsCE.iloc[i])]

    #Combine all derates and then replace placeholder derates
    deratesTechs = pd.concat(deratesTechs,axis=1)
    for c in deratesTechs.columns: capDeratesTechs[c] = deratesTechs[c].values #can't use pd replacement b/c mismatch in date format
...
def derateExistingUnits(genFleet,capDerates,metVars,compressedGens):
    #Set recirculating and dry cooling design assumptions
    rcDesign,dcDesign = setCoolingDesigns()
...
def rowEligForDerates(genRow):
    return ((genRow['PlantType'] in ['Coal Steam','Combined Cycle','Coal Steam CCS','Combined Cycle CCS'] and genRow['CoolingType'] in ['RC','DC']) or (genRow['PlantType'] == 'Combustion Turbine'))
