...
def getSingleREGenTimeseries(genFleet, tgtTz, weatherYears, currYear, pRegionShapes, nonCCReanalysis, climateChange, cesmMember, interconn):
    ...
    #Shift into right tz
    if not climateChange: windGen, solarGen = shiftTz(windGen, tgtTz, currYear, 'wind'),shiftTz(solarGen, tgtTz, currYear, 'solar')
...
def loadData(weatherYears,pRegionShapes,cesmMember,interconn,climateChange,nonCCReanalysis):
    if climateChange: lats, lons, cf, latlonRegion = loadClimateData(weatherYears, pRegionShapes, cesmMember)
        ...
        else: lats, lons, cf, latlonRegion = loadERA5Data(weatherYears, pRegionShapes,interconn) 
...
def loadClimateData(weatherYears, pRegionShapes, cesmMember, dataDir=os.path.join('Data','CESM')):
    print('Using CESM met data!')
    cesmMember = cesmMember[0] if type(cesmMember) == list else cesmMember
    #File and dir
    # solarFile, windFile = path.join(dataDir,'solar_CF_' + cesmMember + '.nc'), path.join(dataDir,'wind_CF_' + cesmMember + '.nc')
    solarFile, windFile = path.join(dataDir,'solar_CF_' + cesmMember + '.nc'), path.join(dataDir,'wind_CF_nonBC_' + cesmMember + '.nc')
    # Error Handling
    ...
    #Put long into -180 to 180 format (needed for sjoin w/ p regions shp)
    latlonPd['lon'] -= 360
...
def loadERA5Data(weatherYears, pRegionShapes, interconn, dataDir=os.path.join('Data','ERA5')):
    ...
    #File and dir
    solarFile, windFile = path.join(dataDir, 'solarCFs_ERA5_' + interconn +  '_' + str(weatherYears[0]) + '.nc'),path.join(dataDir, 'windCFs_ERA5_' + interconn +  '_' + str(weatherYears[0]) + '.nc')
...
def loadNRELData(weatherYears, pRegionShapes):
    ...
    sys.exit('Do not have siting availability datasets for NREL datasets! Use ERA5 instead.')
    # File and dir
    dataDir = 'Data\\RE'
...
def getCFIndexCC(units,lats,lons,cf):
    ...
    for idx,unit in units.iterrows():
        ...
        cfAtClosestPoint = cf['solar'][closestLat][closestLon] #not matter whether wind or solar
