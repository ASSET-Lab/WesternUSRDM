...
def writeTimeDependentConstraints(blockNamesChronoList,stoInCE,seasSto,gamsFileDir,ceOps,
                                 lastRepBlockNames,specialBlocksPrior,removeHydro,hydroGenMonthlyCE):
   setText,setDefns = writeSets(blockNamesChronoList,stoInCE,hydroGenMonthlyCE,removeHydro)
   paramText,paramDefns = writeParameters(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE)
   importText = writeImport(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE) 
   varText = writeVariables(blockNamesChronoList,seasSto) if (len(blockNamesChronoList)>1 or seasSto) else ""
   eqnText = writeEquationsNames(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE)
   eqnText += writeEquations(blockNamesChronoList,stoInCE,seasSto,ceOps,lastRepBlockNames,specialBlocksPrior,removeHydro,hydroGenMonthlyCE)
...
def writeSets(blockNamesChronoList,stoInCE,hydroGenMonthlyCE,removeHydro):
   setText,setDefns = 'Sets\n','\n'
   #Create names for set blocks
   for nB in blockNamesChronoList: setText += '\t' + createHourSubsetName(nB) + '(h)\n'
   #Create names for months (if using them)
   if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame and not removeHydro:
      for idx in hydroGenMonthlyCE.index: setText += '\t' + idx + '(h)\n'
...
def writeParameters(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE):
   ...
   if not removeHydro: 
      ...
      #Max hydro generation by month parameters (if using)
      if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame: 
         for idx in hydroGenMonthlyCE.index: paramText += '\tpMaxgenhydro' + idx + '(z)\n'
   return '\n' + paramText + '\t;\n',paramDefns
...
def writeImport(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE):
   ...
   if not removeHydro: 
      hydroGen = ','.join(['pMaxgenhydro' + createHourSubsetName(nB) for nB in blockNamesChronoList])
      if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame: 
         hydroGen += ',' + ','.join(['pMaxgenhydro' + idx for idx in hydroGenMonthlyCE.index])
         blocks += ',' + ','.join([idx for idx in hydroGenMonthlyCE.index])
   #Combine all text
...
def writeEquationsNames(blockNamesChronoList,stoInCE,seasSto,ceOps,removeHydro,hydroGenMonthlyCE):
   eqnText = '\nEquations\n'
   ...
   if not removeHydro:
      ...
      if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame:
         for idx in hydroGenMonthlyCE.index: eqnText += '\tlimitHydroGen' + idx + '(z)\n'
   return eqnText + '\t;\n'
...
#Write equations
def writeEquations(blockNamesChronoList,stoInCE,seasSto,ceOps,lastRepBlockNames,specialBlocksPrior,removeHydro,hydroGenMonthlyCE):
   eqns = ''
   #Storage equations
   if stoInCE:
      for et in ['storageegu','storagetech']:
   ...
   if not removeHydro:
      ...
      if type(hydroGenMonthlyCE) == pd.core.frame.DataFrame:
         ...
         for idx in hydroGenMonthlyCE.index:
            eqns += 'limitHydroGen{b}(z)..sum((hydroegu,{b})$[pGenzone(hydroegu)=ORD(z)],vGen(hydroegu,{b})) =l= pMaxgenhydro{b}(z);'.format(b=idx) + '\n'
